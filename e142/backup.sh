...
createCodeDump() {
    ...
    *.sh"
    ...
    if [ -n "$DISTRNAMES" ]; then
        echo nice -n 15 tar --exclude=".svn" -czhf $CODEFILENAME $DISTRNAMES
        nice -n 15 tar --exclude=".svn" -czhf $CODEFILENAME $DISTRNAMES
    fi
}

# 4. Create code dump function
createMediaDump() {
    # Content of file archive
    DISTR="
    media
    ...
    if [ -n "$DISTRNAMES" ]; then
        echo nice -n 15 tar --exclude=".svn" -czhf $MEDIAFILENAME $DISTRNAMES
        nice -n 15 tar --exclude=".svn" -czhf $MEDIAFILENAME $DISTRNAMES
    fi
}

# 5. Create DB dump function
createDbDump() {
...
if [ -n "$NAME" ]; then
    ...
    MEDIAFILENAME="$OUTPUTPATH$NAME.tar.gz"
    ...
    CODEFILENAME="$OUTPUTPATH$CODE.$DATETIME.tar.gz"
    MEDIAFILENAME="$OUTPUTPATH$MEDIA.$DATETIME.tar.gz"
    DBFILENAME="$OUTPUTPATH$DB.$DATETIME.sql.gz"
fi
#echo "MODE $MODE"
#echo "Name $NAME"
